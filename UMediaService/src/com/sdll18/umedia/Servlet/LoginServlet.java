...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		User user = getUserService().query(username, false);
		if (user == null) {
			resp.sendRedirect(resp
					.encodeRedirectURL("/UMediaService/Manager/login.html"));
			return;
		}
		if (user.getPassword().equals(password)) {
			Cookie cookie = new Cookie("mark", String.valueOf(user.getId()));
			resp.addCookie(cookie);
			resp.sendRedirect(resp
					.encodeRedirectURL("/UMediaService/Manager/control_info.html"));
		} else {
			resp.sendRedirect(resp
					.encodeRedirectURL("/UMediaService/Manager/login.html"));
		}
